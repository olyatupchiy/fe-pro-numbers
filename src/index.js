...
export const minMaxMultiply = (minFirst, minSecond, maxFirst, maxSecond) => {
  return Math.min(minFirst, minSecond, maxFirst, maxSecond) * Math.max(minFirst, minSecond, maxFirst, maxSecond)
};
...
console.log(getFixedValue(3.2345, 3))
...
export const randomizer = (from, to) => {
  let randomNum = from + Math.random()*(to - from);
